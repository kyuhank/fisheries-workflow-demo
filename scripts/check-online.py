...
def execute(start,handover='connected',previous=None):
 ...
 cursor=0;deadline=time.monotonic()+600;transferred=False;pending=None;events=[]
 while time.monotonic()<deadline:
  ...
  if pending and (pending['boundary']=='data' or any(e.get('job')=='cpue_report' and e['state']=='complete' for e in events)):
   assert update['run']['status']=='handover','Reporting must leave the pending transfer available'
   ...
   request('/transfer',{'connect':False},session);transferred=True;pending=None
  run=update['run']
