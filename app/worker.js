...
async function initialise(payload) {
  ...
  const script = name => URL.createObjectURL(new Blob([decode(assets[name])], {type: 'text/javascript'}));
  self.fetch = async input => {
    const url = typeof input === 'string' ? input : (input.url || String(input));
    const name = url.split('/').pop();
    if (!Object.hasOwn(assets, name)) throw new Error('The preserved runtime does not contain ' + name);
    return new Response(decode(assets[name]), {headers: {'Content-Type': name.endsWith('.wasm') ? 'application/wasm' : 'application/octet-stream'}});
  };
  importScripts(script('pyodide.asm.js'));
  importScripts(script('pyodide.js'));
  python = await loadPyodide({indexURL: 'https://preserved.invalid/', lockFileURL: 'https://preserved.invalid/pyodide-lock.json', stdout: () => {}, stderr: text => console.warn(text)});
...
self.onmessage = async ({data: {id, type, payload}}) => {
  try {
    self.postMessage({id, type: 'response', value: await request(type, payload)});
  } catch (error) {
